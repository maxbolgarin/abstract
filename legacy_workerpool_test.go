...
func TestLegacyWorkerPoolBasicFunctionality(t *testing.T) {
	// Create a worker pool with 5 workers and a queue capacity of 10
	pool := abstract.NewLegacyWorkerPool(5, 10)
	pool.Start()
	defer pool.Stop()
...
func TestLegacyWorkerPoolSubmitWait(t *testing.T) {
	pool := abstract.NewLegacyWorkerPool(3, 5)
	pool.Start()
	defer pool.Stop()
...
func TestLegacyWorkerPoolConcurrency(t *testing.T) {
	// Number of workers and tasks
	workerCount := 10
	taskCount := 100

	pool := abstract.NewLegacyWorkerPool(workerCount, taskCount)
	pool.Start()
	defer pool.Stop()
...
func TestLegacyWorkerPoolTimeout(t *testing.T) {
	// Create a worker pool with a small capacity
	pool := abstract.NewLegacyWorkerPool(1, 1)
	pool.Start()
...
func TestLegacyWorkerPoolShutdown(t *testing.T) {
	pool := abstract.NewLegacyWorkerPool(3, 10)
	pool.Start()
...
func TestSafeLegacyWorkerPool(t *testing.T) {
	pool := abstract.NewSafeLegacyWorkerPool(3, 10)
	pool.Start()
	defer pool.Stop()
...
func TestLegacyWorkerPoolEdgeCases(t *testing.T) {
	// Test with zero or negative parameters
	pool := abstract.NewLegacyWorkerPool(0, -1)
	...
	pool = abstract.NewLegacyWorkerPool(2, 5)
	pool.Start()
	pool.Stop()
